...
<main>
    ...
    <div class="post-box">
        <div class="post-username-box">
            <img src="{{ asset('storage/images/user_icon/' . $post->user->image_at) }}" style="height:60px" alt="">
            <p class="post-username">{{$post->user->name}}</p>
        </div>
        <p class="post-countryname">{{ $post->country_name }}</p>
        <div class="post-images">
            <img src="{{ asset('storage/images/country_img/' . $post->image_1) }}" onclick="image_change()" alt="画像" width="240" height="135" id="image_change">
            {{-- <script>
        ...
        <section class="showpage-comment">
            @foreach ($post->CommentBy() as $comment )
            ...
            <div class="comment-content">
                <div class="comment-icon-userName">
                    ...
                    <p>:</p>
                    <p class="comment">{{ Str::limit($comment->comment_text, 40) . 'read more' }}</p>
